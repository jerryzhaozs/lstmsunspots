...
def extract_nc(path, coord_path, variable_name, precision=3, num_pool=4):
    ...
    lon_dic={}
    for d in lat:
        if round(d,2) in lat_dic:
            print('hell')
        else:
            lat_dic[round(d,2)]=d
    for d in lon:
        if round(d,2) in lon_dic:
            print('hell')
        else:
            lon_dic[round(d,2)]=d
    # print(lon_dic.keys())
    # print(coord["lon"][1])
    # if 115.35 in lon_dic:
    #     print(lon_dic[ 115.35])
    for j in range(len(coord)):
        a=coord["lon"][j]
        closest_value = min((x for x in lon if abs(x - a) <= 0.01), key=lambda x: abs(x-a))
        lon_index.append(closest_value)
        print(j)
        a=coord["lat"][j]
        closest_value = min((x for x in lat if abs(x - a) <= 0.01), key=lambda x: abs(x-a))
        lat_index.append(closest_value)
    #     if coord["lon"][j] in lon_dic:
    #         lon_index.append(lon_dic[coord["lon"][j]])
    #     else:
    #         print(j)

    #     if coord["lat"][j] in lat_dic:
    #         lat_index.append(lat_dic[coord["lat"][j]])
    #     else:
    #         print(j)
    f1.close()
...
if __name__ == "__main__":
    """example"""
    start = time.time()
    # path = "C:/Users/Administrator/Desktop/dogcat/py_nc_jerry/yanxiang/gldas"
    # coord_path = "C:/Users/Administrator/Desktop/dogcat/py_nc_jerry/yanxiang/coord.txt"
    path = "C:/Users/Administrator/Desktop/dogcat/py_nc_jerry/gpm_debug/gpm_1degree"
    coord_path = "C:/Users/Administrator/Desktop/dogcat/py_nc_jerry/gpm_debug/gpm_coord.txt"
    extract_nc(path, coord_path, "HQprecipitation", precision=3)
    end = time.time()
